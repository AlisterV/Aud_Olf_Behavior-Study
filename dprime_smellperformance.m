...
function [y, yodor, FAarray, FAarrayodor, xsig, xsigodor, ysig, ysigodor, threshold, odorthreshold, gohitarray, gohitarrayodor, gomissarray, gomissarrayodor, nogohitarray, nogohitarrayodor, nogomissarray, nogomissarrayodor, trialcounterarray, trialcounterarrayodor] = dprime_smellperformance()
...
x = [0.0000 0.0010 0.0100 0.1000 1.0000];
...
nDistract = repmat(nDistract,length(percorrarray),1)';
nTarget = repmat(nTarget,length(percorrarray),1)';
FAarray2 = repmat(FAarray, length(percorrarray),1)';

%inverts the percent correct array for no odor and saves it as y
y=percorrarray';

ydprimearray=zeros(size(y));

for z=1:numel(y)
    ydprimearray(z)=dprime(y(z),FAarray2(z),nTarget(z),nDistract(z));
end
...
e3=errorbar(log10(x),meandprime',stedprime,'or');
%e4=semilogx(x,meanpercorr','ok');
hold on

title('Mouse avg. d Prime on odor concentrations')
%labels the x axis
xlabel('Log[Conc]')
...
T.Properties.VariableNames=pad({'Concentrations: ','# of Trials: '},'both');
...
% title("Mouse071 1st Session on Odor Concentrations")
% hold on
