...
function performance_graphs_days()
...
switch answer
    ...
    case 'All Files for mouse'
        ...
        day_mat = zeros(length(C),1);
        pHit_mat = zeros(length(C),1);
        pFA_mat = zeros(length(C),1);
        percorr_mat = zeros(length(C),1);
        dpri_mat = zeros(length(C),1);
        ...
        for i=1:numel(C)
            ...
            %if the day contained a training day, the code continues here
                ...
                pHit=GoHitCounterarray/(GoHitCounterarray+GoMissCounterarray);
                %creates a scatter
                %scatter(ax1,day,pHit,140,'MarkerColor','b',)
                plot(ax1,day,pHit,'b-o','MarkerSize',5,'MarkerFaceColor','b','LineWidth',1,'LineStyle','-')
                %labels the x axis
                xlabel(ax1,'Training Day','FontSize',18)
                %labels the y axis
                ylabel(ax1,'Hit Rate','FontSize',18)
                %makes the y tick marks from zeros to one by 0.2
                yticks(ax1,0:.2:1)
                %sets the y limits
                ylim(ax1,[0 1])
                %creates a title
                title(ax1,'Go Hit Rate','FontSize',24)
                %holds onto all plots
                hold([ax1,ax2,ax3,ax4],'on')
                %calculates the percent false alarm rate
                pFA=NoGoMissCounterarray/(NoGoHitCounterarray+NoGoMissCounterarray);
                %creates a scatter
                %scatter(ax2,day,pFA,140,'ro-')
                plot(ax2,day,pFA,'-or','MarkerSize',5,'MarkerFaceColor','r','LineWidth',1,'LineStyle','-')
                %labels the x axis
                xlabel(ax2,'Training Day','FontSize',18)
                %labels the y axis
                ylabel(ax2,'F.A. Rate','FontSize',18)
                %makes the y tick marks from zeros to one by 0.2
                yticks(ax2,0:.2:1)
                %sets the y limits
                ylim(ax2,[0 1])
                %creates a title
                title(ax2,'False Alarm Rate','FontSize',24)
                %holds onto all plots
                hold([ax1,ax2,ax3,ax4],'on')
                %calculates the target number of pHit rate
                nTarget=(GoHitCounterarray+GoMissCounterarray);
                ...
                [dpri]=dprime(pHit,pFA,nTarget,nDistract);
                %creates a scatter
                %scatter(ax4,day,dpri,140,'go-')
                plot(ax4,day,dpri,'-og','MarkerSize',5,'MarkerFaceColor','g','LineWidth',1,'LineStyle','-')
                %labels the x axis
                xlabel(ax4,'Training Day','FontSize',18)
                %labels the y axis
                ylabel(ax4,"d'",'FontSize',18)
                %creates a title
                title(ax4,"D-prime",'FontSize',24)
                %holds onto all plots
                hold([ax1,ax2,ax3,ax4],'on')
                %calculates the entire percent correct
                percorr=(GoHitCounterarray+NoGoHitCounterarray)/(sum(alltrialcounter,'all'));
                %creates a scatter
                %scatter(ax3,day,percorr*100,'MarkerSize',15,'mo-')
                plot(ax3,day,percorr*100,'-om','MarkerSize',5,'MarkerFaceColor','m','LineWidth',1,'LineStyle','-')
                %labels the x axis
                xlabel(ax3,'Training Day','FontSize',18)
                %labels the y axis
                ylabel(ax3,'Percentage (%)','FontSize',18)
                %makes the y tick marks from zeros to one by 0.2
                yticks(ax3,0:20:100)
                %sets the y limits
                ylim(ax3,[0 100])
                %creates a title
                title(ax3,'Total Percent Correct','FontSize',24)
                %holds onto all plots
                hold([ax1,ax2,ax3,ax4],'on')
                %creates an overall title for all graphs
                sgtitle("Performance of Mouse "+convertCharsToStrings(mousenum)+" with Normalized Test Days")
            ...
            pHit_mat(i,1) = pHit;
            pFA_mat(i,1) = pFA;
            percorr_mat(i,1) = percorr*100;
            dpri_mat(i,1) = dpri;
        end
        plot(ax1,day_mat,pHit_mat(:,:),'b')
        plot(ax2,day_mat,pFA_mat(:,:),'r')
        plot(ax3,day_mat,percorr_mat(:,:),'m')
        plot(ax4,day_mat,dpri_mat(:,:),'g')
