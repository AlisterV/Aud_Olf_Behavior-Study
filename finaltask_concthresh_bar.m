function [odor_thresh_array,sess_array] = finaltask_concthresh_bar(x_sound,x_sound_size,x_conc,x_conc_size,theFiles)

%% SOUND VS. ODOR

holdx = x_conc;

odor_thresh_array = zeros(x_sound_size,length(theFiles));
...
for v = 1:x_sound_size
    ...
    for k = 1 : length(theFiles)
        ...
        trialcounterarray=zeros(x_conc_size,1);
    ...
    %the x data
    odor_thresh_array(v,k) = odorthreshold;
    end
end

avg_odor_thresh_array = mean(odor_thresh_array,2);

std_thresh = std(odor_thresh_array,0,2);
[row,column]=size(odor_thresh_array);
stecorr = std_thresh./(sqrt(column));
figure()
hold on
bar(x_sound,avg_odor_thresh_array)
hold on
errorbar(x_sound,avg_odor_thresh_array,stecorr,'LineStyle', 'none')
xlabel('Sound Level')
ylabel('Odor threshold')
title('Odor Thresholds at Specific Sound Levels')
end