function [sound_thresh_array,sess_array] = finaltask_soundthresh_bar(x_sound,x_sound_size,x_conc,x_conc_size,theFiles)

%% ODOR VS. SOUND

holdx = x_sound;

sound_thresh_array = zeros(x_conc_size,length(theFiles));
...
for v = 1:x_conc_size
    ...
    %the x data
    sound_thresh_array(v,k) = soundthreshold;
    end
end

avg_sound_thresh_array = mean(sound_thresh_array,2);

std_thresh = std(sound_thresh_array,0,2);
[row,column]=size(sound_thresh_array);
stecorr = std_thresh./(sqrt(column));
figure()
hold on
bar(x_conc,avg_sound_thresh_array)
hold on
errorbar(x_conc,avg_sound_thresh_array,stecorr,'LineStyle', 'none')
xlabel('Concentration')
ylabel('Sound threshold')
title('Sound Thresholds at Specific Concentrations')
end