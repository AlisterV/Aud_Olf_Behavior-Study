...
function [y, yodor, FAarray, FAarrayodor, xsig, xsigodor, ysig, ysigodor, threshold, odorthreshold, gohitarray, gohitarrayodor, gomissarray, gomissarrayodor, nogohitarray, nogohitarrayodor, nogomissarray, nogomissarrayodor, trialcounterarray, trialcounterarrayodor] = any_conc_check()
...
for s = 1 : length(theFiles)
    ...
    odr = h5read(filehold, '/Trials');
    %gets the sound level data and find only the unique elements
    %(eliminates repeats)
    odr.concentration = []';
    odors = string(odr.odorant');
    for g = 1:length(odr.trialNumber)
     conc_str = extractAfter(odors(g),'_');
     conc1 = str2double(conc_str);
     if isnan(conc1)
         conc1 = 0;
     end
     flows = [odr.air_flow(g) odr.nitrogen_flow(g)];
     conc2 = flows(2)/(flows(1)+flows(2));
     conc = conc1*conc2;
     odr.concentration(g) = conc;
    end
    odr.concentration = odr.concentration';
    or = {unique(odr.concentration)};
...
for t = 1:length(theFiles)
    curr_len = length(cell2mat(odorhold(t)));
    if curr_len > len
        len = curr_len;
    elseif curr_len < len
        len = len;
...
odrs = zeros(length(theFiles),len);
for d = 1:length(theFiles)
    curr_lev = cell2mat(odorhold(d))';
...
new_concs = zeros(length(theFiles), len);
...
for k = 1 : length(theFiles)
  ...
  odorresponse=zeros(numel(z),NumTrials);
  
  trialcounter=zeros(numel(z),NumTrials);
