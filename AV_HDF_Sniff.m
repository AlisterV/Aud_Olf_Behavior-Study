...
function AV_HDF_Sniff()

close all;
clear all;
clc;

windowSize = 100; % Setting parameters/window of the moving filter that happens later on, in ms. Try to keep to a range of 5-50ms based on literature.
...
% filePattern = fullfile(myFolder, '*.h5');
% %opens user access to the desired folder 
% NameFile = uigetfile(filePattern);
%avg_prom_all=zeros(length(theFiles),1);
%percorr_all=zeros(length(theFiles),1);
array=zeros(length(theFiles),3);
T=array2table(array);
%T=table({length(theFiles)},length(percorr_all),length(avg_prom_all));

for w=1:length(theFiles)
    %selects the kth file
    NameFile = theFiles(w).name;
    ...
    if Scanner ~= 1
        ...
        hold on
        findpeaks(movFiltered,CorrAxis,'MinPeakDistance',(1*10^2.5))
        [pks,locs,widths,proms] = findpeaks(movFiltered,CorrAxis,'MinPeakDistance',(1*10^2.5));
        avg_prom = mean(proms);
        hold off
...
T.Properties.VariableNames = {'Session #', 'Percent Correct', 'Avg. Prominance'};
T
T=T{:,2:end};
plot(T(:,2),T(:,1))
end
