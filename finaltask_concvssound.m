function finaltask_concvssound(x_sound,x_sound_size,x_conc,x_conc_size,theFiles)
...
for v = 1:x_conc_size
    ...
    gohitarray=zeros(x_sound_size,length(theFiles));
    %gohitarrayodor=zeros(numel(x_conc),length(theFiles));
    nogohitarray=zeros(x_sound_size,length(theFiles));
    %nogohitarrayodor=zeros(numel(x_conc),length(theFiles));
    gomissarray=zeros(x_sound_size,length(theFiles));
    %gomissarrayodor=zeros(numel(x_conc),length(theFiles));
    nogomissarray=zeros(x_sound_size,length(theFiles));
    %nogomissarrayodor=zeros(numel(x_conc),length(theFiles));
    trialcounterarray=zeros(x_sound_size,length(theFiles));
    ...
    for k = 1 : length(theFiles)
        ...
        trialcounter=zeros(x_sound_size,NumTrials);
        %trialcounterodor=zeros(numel(x_conc),NumTrials);
        soundresponse = zeros(x_sound_size,NumTrials);
    ...
    [soundthresh] = finaltask_soundonly(x_sound, x_sound_size,theFiles);
    hold on
...
%     hold on
    ...
    hold on
    e3=errorbar(x_sound,meanpercorr',stecorr,'ob');
    ...
    title("Sound Trials w/ Conc. " +convertCharsToStrings(curr_conc) + " for Mouse: "+convertCharsToStrings(mousenum) + "; " + "Session: " + convertCharsToStrings(Data.session(1)));
    hold on
    ...
    [params,mdl,soundthreshold,sensitivity,fmcon,minfun,pthresh] = fitLogGridsound(x_sound(:),y_sound(:));
    %creates 100 data points ranging from the smallest to the largest value in
    %the x data
    xf=linspace(min(x_sound(:)),max(x_sound(:)),100);
    hold on
    ...
    %name
    sound_thresh_array(1,v) = soundthreshold;
    e8=xline(soundthreshold,'--b','DisplayName',"Threshold = "+convertCharsToStrings(soundthreshold)+" dB");%,'LabelHorizontalAlignment','right','LabelVerticalAlignment','bottom')
    ...
    eleg=legend([e8 soundthresh],'location','best');
end
end