function [soundthresh] = finaltask_soundonly(x_sound, x_sound_size,theFiles)
...
e3=errorbar(x_sound,meanpercorr',stecorr,'or');
...
%title("Sound Only Trials for Mouse: "+convertCharsToStrings(mousenum) + "; " + "Session: " + convertCharsToStrings(Data.session(1)));
hold on
...
%name
soundthresh=xline(soundthreshold,'--r','DisplayName',"Sound Only Threshold = "+convertCharsToStrings(soundthreshold)+" dB");%,'LabelHorizontalAlignment','right','LabelVerticalAlignment','bottom')
...
eleg=legend(soundthresh,'location','best');
