...
%as no go trial)
FAarray=FAarray(1,:);
FAarray = repmat(FAarray,length(percorrarray),1)';
%only takes the top row of false alarm data for odor (only works for 0dB as
%no go trial)
FAarrayodor=FAarrayodor(1,:);
FAarrayodor = repmat(FAarrayodor,length(percorrarrayodor),1)';
...
nDistractodor = repmat(nDistractodor,length(percorrarray),1)';
nTarget = repmat(nTarget,length(percorrarray),1)';
nTargetodor = repmat(nTargetodor,length(percorrarrayodor),1)';
% %creates a counter
% columncounter=0;
...
ydprimeodorarray=zeros(size(yodor));
